...
#library(coda,lib.loc="/proj/cdjones_lab/weiyq/R/x86_64-redhat-linux-gnu-library/3.6")
#library(parallel,lib.loc="/proj/cdjones_lab/weiyq/R/x86_64-redhat-linux-gnu-library/3.6")
library(coda)
library(parallel)
# need to install this library for rdirichlet function
# install.packages("bmixture", repos='http://cran.us.r-project.org')
#library(bmixture,lib.loc="/proj/cdjones_lab/weiyq/R/x86_64-redhat-linux-gnu-library/3.6")
#library(fields,lib.loc="/proj/cdjones_lab/weiyq/R/x86_64-redhat-linux-gnu-library/3.6")
library(bmixture)
library(fields)
...
freqplot1 <- function( DataFile, Sites, SaveFolder, M = 100 ){ #Three time points
  ...
  for (Site in Sites){
    sitedata = rawdata[ , (0 : 3) * n + Site ] #0:2
...
##################################
#LP(Z0,seq(1,K,1),length(Gp),1/((nrow(rawdata))^2))
# equation (1) in paper
...
#This function removes the treatment test from rawdata and pastes it at the end of the rawdata. 
## Gerneralizes code to more than one type of experiemntal set up
## Parameters
### tests: total number of time points
### PostD: time point with the treatment

unconcatenate<-function(tests,PostD,rawdata){
  n=ncol(rawdata)/tests
  print(n)  #delete me
  start<-n*(PostD-1)+1
  print(start) #delete me
  stop<-n*(PostD)
  print(stop) #delete me
  Treatment<-rawdata[,c(start:stop)]
  NonTreatment<-rawdata[,-c(start:stop)]
  rawdatanew<-cbind(NonTreatment,Treatment)
  return(rawdatanew)}

#Save warnings and errors into .Rdata
Catch_Error <- function(expr) {
  ...
  my_error=list(value=value, warning=warn, error=err)
  save(my_error, file=paste('./Tests/my_error_',copy, '.Rdata',sep=""))
  print("saved errors!")
}
